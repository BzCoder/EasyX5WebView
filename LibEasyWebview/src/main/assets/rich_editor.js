...
RE.setShow = function (contents) {
    window.control.log('设定HTML内容' + contents);
...
RE.changeFontSize = function (tag,size) {
    ...
    for (var i = 0; i < Nodes.length; i++) {
        console.log(tag+"数量:" + Nodes.length);
        Nodes[i].style.fontSize = sizeType;
...
//获取图片列表
RE.getImageList = function () {
    var imgs = document.getElementsByTagName("img");
    console.log("getImageList遍历图片数量:" + imgs.length);
    var array = new Array();
    for (var j = 0; j < imgs.length; j++) {
        array[j] = imgs[j].src;
    }
    window.control.getImageList(array);
}
//设置某个标签字体
RE.setTagFontFamily = function (tag) {
    var Nodes = document.getElementsByTagName(tag);
    for (var i = 0; i < Nodes.length; i++) {
        Nodes[i].style.fontFamily = "myFont";
    }
}

//去除某个标签Style
RE.removeTagStyle = function (tag) {
    var Nodes = document.getElementsByTagName(tag);
    for (var i = 0; i < Nodes.length; i++) {
